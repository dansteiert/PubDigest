...
def selective_term_similarity_heatmap(config: dict, query_term: str,
    ...
    v_min = min(0, df["similarity"].head(12*max_entries).min())
...
def selective_term_similarity_heatmap_publication(config: dict, query_term: str,
                                                  figure_ratio: float = 3 / 4, figure_scale: float = 1,
                                                  fontsize: int = 10,
                                                  titlesize: str = "xx-large"
                                                  ):
    logger = logging.getLogger(config["System"]["logging"]["logger_name"])
    ...
    set_figure_estetics(figure_ratio=figure_ratio, figure_scale=figure_scale, fontsize=fontsize, titlesize=titlesize)
    base_file = os.path.join(config["System"]["base_dir"], "Data_for_Plotting", "Embedding",
                             # config["NLP"]["prediction_term_category"],
                             f"specific_term_validation_{config['NLP']['skip_gram_CBOW']}_"
                             f"{config['Query']['prediction_term']}_{query_term}.pkl")
    ...
    df_occurrence = df_occurrence[df_occurrence.index.isin(set(df["query_term"]))]
    v_min = min(0, df["similarity"].head(12 * max_entries).min())
    v_max = max(0.5, df["similarity"].max())

    df_merged = pd.merge(df, df_occurrence, left_on="query_term", right_on="Term", how="left")

    max_entries = 6
    for idx, terms in enumerate([*config["Query"]["terms_for_embedding_evaluation"],
                                 [i for j in config["Query"]["terms_for_embedding_evaluation"] for i in j]]):
        target_file = os.path.join(config["System"]["base_dir"], "Plot", "Embedding",
                                   f"specific_term_validation_{config['NLP']['skip_gram_CBOW']}_"
        ...
        if idx == 0:
            selected_terms = set(terms)
            df_temp = df_merged[df_merged["query_term"].isin(selected_terms)]
        else:
            selected_terms = set(terms)
            df_temp = df_merged[
                df_merged["query_term"].isin(selected_terms) & (~df_merged[config["Query"]["base_query_abb"]])]

        if df_temp.shape[0] == 0:
            continue
        df_temp = df_temp.sort_values(by="similarity", ascending=False)
        # print(idx, df_temp)
        df_temp = df_temp.head(max_entries)
        # print(idx, df_temp)

        fig, (ax1, ax2) = plt.subplots(1, 2, gridspec_kw={'width_ratios': [6, 4]},
                                       **{"figsize": (6, (2 / 6) * (max_entries * 0.95))})

        df_g2 = df_temp[[config["Query"]["base_query_abb"], *config["Query"]["alternate_query_abb"]]]
        g2 = sns.heatmap(df_g2, cbar=False, cmap="Blues", linecolor="0.5", linewidths=0.5, vmin=0, vmax=1, ax=ax2)
        g2.tick_params(top=False, labeltop=False, bottom=True, labelbottom=True, labelleft=False, left=True)
        g2.set_ylabel("")
        if config["Visualisations"]["with_title"]:
            g2.set_title("Term occurrence\nwithin dataset")

        df_g1 = df_temp.set_index("query_term").rename(
            columns={"similarity": "Term similarity"})[["Term similarity"]]
        g1 = sns.heatmap(data=df_g1,
                         cmap="coolwarm", cbar=False, annot=True, fmt=".3f", linecolor="0.5", linewidths=0.5,
                         ax=ax1,
                         vmin=v_min, vmax=v_max)
        g1.tick_params(top=False, labeltop=False, bottom=True, labelbottom=True, labelleft=True, left=True)
        g1.tick_params(axis="y", rotation=0)
        make_axes_area_auto_adjustable(ax=ax1, pad=0.3, adjust_dirs=["left"])
        ...
        plt.tight_layout(w_pad=3.8, rect=(0, 0, 0.96, 1))
        # plt.show()
        save_plt_figure(path=target_file, logger=logger, save_as_svg=True)
